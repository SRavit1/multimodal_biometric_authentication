...
def conv3x3(in_planes: int, out_planes: int, stride: int = 1, groups: int = 1, dilation: int = 1, act_bw=1, weight_bw=1) -> nn.Conv2d:
...
def conv1x1(in_planes: int, out_planes: int, stride: int = 1, act_bw=1, weight_bw=1) -> nn.Conv2d:
    ...
    return binarized_modules.QuantizeConv2d(in_planes, 
        ...
        bias=False, bitwidth=act_bw, weight_bitwidth=weight_bw)
...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        activation_type="relu", leaky_relu_slope=0.1
    ) -> None:
        super().__init__()
